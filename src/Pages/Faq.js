...
import { get_api_index_campaign, get_api_index_faq_pagination, get_api_store_faq, get_public_url } from "../Utils/Utils";
...
function Faq() {
  ...
  function getFaq() {
    ...
    fetch(get_api_index_faq_pagination, {
      method: "POST",
      body: Form,
    })
      .then(
  ...
  //   if (faqApi && faqApi.status == "success") {
  //     SetPage(faqApi.pagination.current_page);
  //     SetTotalPage(faqApi.pagination.total);
  //     SetPagePrev(faqApi.pagination.previous_page);
  //     SetPageAfter(faqApi.pagination.next_page);
  //     return faqApi.data.map((value, index) => {
  //       return (
  //         <div className="container-list-text" key={index}>
  //           <p><Link className="text-black text-decoration-none" to={`/faq/detail/` + value.id_faq}>{value.title_faq}</Link></p>
  //           <p style={{ color: "#9b9e9c" }}>{value.tanggal_buat_faq}</p>
  //         </div>
  //       );
  //     });
  //   } else {
  ...
  function ContentList() {
    if (faqApi && faqApi.status === "success") {
      ...
      const sortedData = faqApi.data.sort((a, b) => new Date(b.tanggal_buat_faq) - new Date(a.tanggal_buat_faq));
    ...
    if (levelUsersLogin == "Admin") {
      return (
        <button type="submit" className="btn fw-bold w-100" style={{
          ...
          onMouseOut={(e) => e.target.style.backgroundColor = "#F7A600"}>Tambahkan FAQ</button>
      );
    }
  }
    ...
    <div className="App">
      ...
      <Modal show={show} onHide={handleClose} size="xl">
        <Modal.Header className="border-bottom border-dark" style={{ backgroundColor: "#DCE1E7" }} closeButton>
          <Modal.Title className="text-center w-100">FAQ</Modal.Title>
        </Modal.Header>
        ...
        <Modal.Footer className="d-flex justify-content-center border-top-0" style={{ backgroundColor: "#DCE1E7" }}>
          ...
          <Button className="btn-lg btn-warning px-5 fw-bold" variant="warning" onClick={(e) => handleSubmitFaq(e)}>
            Submit Form
          </Button>
        </Modal.Footer>
      </Modal>
